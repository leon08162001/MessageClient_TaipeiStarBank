...
//#endif
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        Common.LogHelper.MoneySQLogger.LogError<BaseEMSAdapter>(ex);
                        throw ex;
                    }
                    try
                    {
        ...
        public bool SendEMSMessage(string MessageIDTag, List<MessageField> SingleEMSMessage, int DelayedPerWhenNumber = 0, int DelayedMillisecond = 0)
        {
            ...
            try
            {
                ...
                if (_Session != null)
                {
                    ...
                    {
                        Common.LogHelper.MoneySQLogger.LogInfo<BaseEMSAdapter>("Network connection or TibcoEMSService Has been closed!");
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                Common.LogHelper.MoneySQLogger.LogError<BaseEMSAdapter>(ex, ErrorMsg);
        ...
        private void _Connection_ExceptionHandler(object sender, EMSExceptionEventArgs args)
        {
            ...
            if (ex.Message.Equals("Connection unknown by server"))
            {
                ...
                Common.LogHelper.MoneySQLogger.LogError<BaseEMSAdapter>(string.Format(ex.Message + "(Connection has performed fault-tolerant switch to {0})", ConnActiveUrl));
            }
            else
            {
                Common.LogHelper.MoneySQLogger.LogError<BaseEMSAdapter>(ex);

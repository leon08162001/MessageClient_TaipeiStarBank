        ...
        public bool CheckActiveMQAlive()
        {
            ...
            if (urls.IndexOf(",") == -1)
            {
                ...
                if (ports.IndexOf(",") == -1)
                {
                    ...
                    {
                        Common.LogHelper.MoneySQLogger.LogError<BaseMQAdapter>(ex, "IsActiveMQAlive() Error");
                        result = false;
                    }
                }
                //代表多個port
...
//#endif
//#if IOS
//                _MacAddress = Util.GetIosMacAddress();
//#endif
            }
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        Common.LogHelper.MoneySQLogger.LogError<BaseMQAdapter>(ex);
                        throw ex;
                    }
                    try
                    {
        ...
        public void listener_messageReceivedEventHandler(IMessage message)
        {
            ...
            {
                Common.LogHelper.MoneySQLogger.LogInfo<BaseMQAdapter>("MQMessage Format is InCorrect");
            }
            else
            {
        ...
        public bool SendMQMessage(string MessageIDTag, List<MessageField> SingleMqMessage, int DelayedPerWhenNumber = 0, int DelayedMillisecond = 0)
        {
            ...
            catch (Exception ex)
            {
                ...
                Common.LogHelper.MoneySQLogger.LogError<BaseMQAdapter>(ex, ErrorMsg);
        ...
        public bool SendFile(string FileName, string FilePath, string ID = "")
        {
            ...
            try
            {
                if (_Producer != null)
                {
                    ...
                    {
                        Common.LogHelper.MoneySQLogger.LogInfo<BaseMQAdapter>("Network connection or ActiveMQService Has been closed!");
                    }
                }
            }
            catch (Exception ex)
                ...
                if (ListenName != "" && _Session != null)
                {
                    ...
                    else if (_DestinationFeature == DestinationFeature.VirtualTopic)
                    {
                        ...
                        for (int i = 0; i < _VirtualTopicConsumers; i++)
                        {
                            //if (_Selector.Equals(""))
                            //{
                            //    QueueConsumer[i] = _Session.CreateConsumer(new ActiveMQQueue(_VirtualTopic.Replace("*", i.ToString() + "_" + _Connection.ClientId) + _ListenName));
                            //}
                            //else
                            //{
                            //    QueueConsumer[i] = _Session.CreateConsumer(new ActiveMQQueue(_VirtualTopic.Replace("*", i.ToString() + "_" + _Connection.ClientId) + _Connection.ClientId + _ListenName), _Selector);
                            //}
                            if (_Selector.Equals(""))
                            {
        ...
        {
            //int iSlowDownNum = this.SenderDBAction == DBAction.Query ? 10 : this.SenderDBAction == DBAction.Add ? 8 : this.SenderDBAction == DBAction.Update ? 8 : this.SenderDBAction == DBAction.Delete ? 20 :
            //                   30;
        ...
        void _Connection_ConnectionResumedListener()
        {
            string ConnActiveUrl = (_Connection as Apache.NMS.ActiveMQ.Connection).ITransport.RemoteAddress.AbsoluteUri;
            Common.LogHelper.MoneySQLogger.LogInfo<BaseMQAdapter>(string.Format("Connection has performed fault-tolerant switch to {0}", ConnActiveUrl));
        }
    }
}

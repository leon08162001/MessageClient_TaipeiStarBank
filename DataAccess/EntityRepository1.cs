...
namespace DataAccess
{
    ...
    /// 實體化資料庫物件模型存取類別
    /// </summary>
    public class EntityRepository1
    {
        private static readonly log4net.ILog log = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);
        protected readonly DbContext _DbContext;

        public EntityRepository1(DbContext DbContext)
        {
        ...
        /// <param name="strSort"></param>
        /// <returns></returns>
        public virtual List<T> GetTop<T>(int Number, string strSort = null) where T : class, new()
        {
            List<T> query;
            if (strSort != null && strSort.Any())
            {
        ...
        {
            List<T> query = _DbContext.Set<T>().Where(WherePredicate).AsEnumerable<T>().ToList();
            if (SortKeys != null && SortKeys.Any())
            {
                query.SortBy<T>(SortKeys);
            }
        ...
        public virtual List<T> Find<T>(List<T> TEntitySet, params Expression<Func<T, IComparable>>[] SortKeys) where T : class, new()
        {
            ...
            if (TEntitySet.Count() > 0)
            {
                ...
                {
                    Common.LogHelper.MoneySQLogger.LogError<EntityRepository1>(ex);
                }
            }
            return query;
        }
        ...
        private RawSqlString GenerateSQL<T>(T TEntity) where T : class, new()
        {
            ObjectContext objectContext = new ObjectContext(_DbContext.Database.GetDbConnection().ConnectionString);
            ObjectSet<T> set = objectContext.CreateObjectSet<T>();
        ...
        private RawSqlString GenerateCountSQL<T>(Dictionary<string, object> PkValueList) where T : class, new()
        {
            ObjectContext objectContext = new ObjectContext(_DbContext.Database.GetDbConnection().ConnectionString);
            ObjectSet<T> set = objectContext.CreateObjectSet<T>();
            string tableName = set.EntitySet.ElementType.Name;
